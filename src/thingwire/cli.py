...
@click.option("--ha-discovery", is_flag=True, default=False, help="Publish Home Assistant MQTT discovery messages")
def serve(broker: str | None, port: int | None, transport: str, log_level: str, ha_discovery: bool) -> None:
    ...
    if ha_discovery:
        os.environ["THINGWIRE_HA_DISCOVERY"] = "1"
...
// --- Generated by: thingwire firmware-config ---
...
#define TELEMETRY_INTERVAL_MS {telemetry_interval_ms}
...
@main.command(name="firmware-config")
@click.argument("yaml_file", type=click.Path(exists=True))
@click.option("--output", "-o", default=None, help="Output path (default: firmware/src/config.h)")
def firmware_config(yaml_file: str, output: str | None) -> None:
    """Generate firmware config.h from a YAML device configuration."""
    import yaml
    ...
    content = CONFIG_H_TEMPLATE.format(
        dht_pin=pins.get("dht", 4),
        pir_pin=pins.get("pir", 5),
        relay_pin=pins.get("relay", 12),
        led_pin=pins.get("led", 13),
        mqtt_broker=mqtt.get("broker", "192.168.1.100"),
        mqtt_port=mqtt.get("port", 1883),
        device_id=cfg.get("device_id", "thingwire-demo-001"),
        telemetry_interval_ms=cfg.get("telemetry_interval_ms", 5000),
        wifi_ssid=wifi.get("ssid", ""),
        wifi_password=wifi.get("password", ""),
    )

    out_path = output or "firmware/src/config.h"
    os.makedirs(os.path.dirname(out_path) or ".", exist_ok=True)
    with open(out_path, "w") as f:
        f.write(content)

    click.echo(f"Generated {out_path} from {yaml_file}")
    click.echo(f"  Device: {cfg.get('device_id', 'thingwire-demo-001')}")
    click.echo(f"  Broker: {mqtt.get('broker', '192.168.1.100')}:{mqtt.get('port', 1883)}")
    click.echo(f"\nFlash with: cd firmware && pio run --target upload")
